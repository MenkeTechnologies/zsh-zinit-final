#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-zinit-final — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `zsh-zinit-final.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='zsh-zinit-final.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/zsh-zinit-final.plugin.zsh"
    assert $state equals 0
}
